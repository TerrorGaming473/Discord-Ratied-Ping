...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'psychotic' | 'bad-friend' | 'angel-heart':
					curStage = 'psicotico';
				case 'demon':
					curStage = 'demon';
				case 'boby-barbershop':
					curStage = 'barbershop';
				case 'demoniac-boby':
					curStage = 'cursed';
				default:
		...
		{
			case 'psicotico':
				var psicotico:BGSprite = new BGSprite('backgrounds/psicotico', -600, -300, 0.9, 0.9);
				add(psicotico);
			case 'demon':
				var demon:BGSprite = new BGSprite('backgrounds/demon', -600, -300, 0.9, 0.9);
				add(demon);
		}
		...
		if(gfVersion == null || gfVersion.length < 1) {
			switch (curStage)
			{
				case 'psicotico' | 'demon' | 'barbershop' | 'cursed':
					gfVersion = 'gfcanibal';
				default:
					gfVersion = 'gfcanibal';
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			...
			scoreTxt.text = 'Score: ' + songScore\n + 'Misses: ' + songMisses\n + 'Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - '\n + ratingFC;//peeps wanted no integer rating
		}
				...
				switch(achievementName)
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable an d you can add weeks with different names than the achievement tag
							{
								case 'week1':
